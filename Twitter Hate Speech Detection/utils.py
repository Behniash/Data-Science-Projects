"""
Twitter Hate Speech Detection - Utility Functions
...
def load_data(filepath="data/labeled_data.csv"):
    """Load Davidson hate-speech data; map numeric class to readable names."""
    df = pd.read_csv(filepath)
    class_map = {0: 'hate_speech', 1: 'offensive', 2: 'neither'}
    df['class_name'] = df['class'].map(class_map)
    return df[['tweet', 'class_name']]
